Describe "Validating the module manifest" {
	...
	$manifest = ((Get-Content "$moduleRoot\fscps.ascii.psd1") -join "`n") | Invoke-Expression
	[version]$moduleVersion = Get-Item "$moduleRoot\fscps.ascii.psm1" | Select-String -Pattern '\$script:ModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
