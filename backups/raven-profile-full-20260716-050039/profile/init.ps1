...
# Add Raven repo psmodules to PSModulePath
try {
    $repoModulePath = Join-Path $global:RavenRepoRoot "psmodules"

    if (Test-Path $repoModulePath) {
        $paths = $env:PSModulePath -split [IO.Path]::PathSeparator

        if ($paths -notcontains $repoModulePath) {
            $env:PSModulePath = ($repoModulePath + [IO.Path]::PathSeparator + $env:PSModulePath)
        }
    }
}
catch {}
